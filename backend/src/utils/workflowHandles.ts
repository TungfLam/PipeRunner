import type { WorkflowEdge, WorkflowInputDefinition, WorkflowNode, WorkflowOutputDefinition } from "../types/domain";
...
function shouldAutoUpdateName(currentName: string | undefined, previousFlag: string | undefined, fallback: string) {
  return !currentName || currentName === previousFlag || currentName === nameFromFlag(previousFlag, fallback);
}

function normalizeInputDefinition(input: WorkflowInputDefinition, index: number): WorkflowInputDefinition {
...
function normalizeOutputDefinition(output: WorkflowOutputDefinition, index: number): WorkflowOutputDefinition {
...
function normalizeNodeHandles(node: WorkflowNode): WorkflowNode {
...
function resolveInputHandle(node: WorkflowNode, handle?: string) {
...
function resolveOutputHandle(node: WorkflowNode, handle?: string) {
...
function hasInputHandle(node: WorkflowNode, handle?: string) {
  return !handle || (node.inputs || []).some((input) => input.name === handle);
}

function hasOutputHandle(node: WorkflowNode, handle?: string) {
  return !handle || (node.outputs || []).some((output) => output.name === handle);
}

function edgeId(edge: WorkflowEdge) {
  return `edge-${edge.source}-${edge.sourceHandle || ""}-${edge.target}-${edge.targetHandle || ""}`;
}

export function repairWorkflowGraphHandles<T extends { nodes: WorkflowNode[]; edges: WorkflowEdge[] }>(workflow: T): T {
  ...
  const edges = workflow.edges.flatMap((edge) => {
    ...
    const repairedEdge: WorkflowEdge = {
      ...edge,
